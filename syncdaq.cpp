...
enum StreamFormat { FORMAT_CF32, FORMAT_CS16 };
...
public:
  ...
  SyncdaqSDR() = delete;
  SyncdaqSDR(const std::string ip_str, const int local_port, int port_id,
             const std::vector<uint32_t> &shifts1, int32_t firshift1,
...
public:
  std::string getDriverKey(void) const { return "syncdaq"; }
  ...
  void setFrontendMapping(const int direction,
  ...
  Kwargs getChannelInfo(const int direction,
  ...
  SoapySDR::RangeList
  getFrequencyRange(const int direction, const size_t channel,
  ...
  ArgInfoList getFrequencyArgsInfo(const int direction,
                                   const size_t channel) const override {
    ArgInfoList result;
    ArgInfo ai;
    ai.key = "lo_freq";
  ...
  virtual void setFrequency(const int direction, const size_t channel,
  ...
  std::vector<std::string>
  listFrequencies(const int direction, const size_t channel) const override {
  ...
  setupStream(const int direction, const std::string &format,
    ...
    if (format == SOAPY_SDR_CF32) {
      // throw std::runtime_error(std::format("syncdaq only support CF32, not
      // {}", format));
      ...
      throw std::runtime_error(
          std::format("syncdaq only support CF32 and CS16, not {}", format));
    }
    ...
    // if (iter != args.end())
    // {
    //     // std::cout << "lo ch not given, use default value 1024" <<
    //     std::endl;
    ...
    setup_data_stream(device_handler.get(), shifts.data(), shifts.size(),
                      firshift);
    return (Stream *)this;
  }

  int activateStream(Stream *stream, const int flags = 0,
    ...
    std::cout << "sample rate: " << getSampleRate(SOAPY_SDR_RX, 0) << std::endl;
    return 0;
  }

  void closeStream(Stream *stream) override { deactivateStream(stream, 0, 0); }

  int deactivateStream(Stream *stream, const int flags = 0,
  ...
  size_t getStreamMTU(Stream *stream) const override { return get_mtu(); }

  int readStream(Stream *stream, void *const *buffs, const size_t numElems,
...
SoapySDR::KwargsList findSyncdaqSDR(const SoapySDR::Kwargs &args) {
  ...
  if (ctrl_ip == nullptr) {
    ...
    if (device_is_alive(parse_ipv4(ctrl_ip))) {
      std::cout << "device with ctrl ip " << ctrl_ip << " is alive"
                << std::endl;
      SoapySDR::Kwargs args1;
 ...
 * Registration
 **********************************************************************/
static SoapySDR::Registry registerSyncdaqSDR("syncdaq", &findSyncdaqSDR,
                                             &makeSyncdaqSDR,
