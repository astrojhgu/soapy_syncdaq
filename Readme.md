...
|port_id|`[0,7]`|射频端口编号|
|shifts|`"s1:s2:s3..."`|注意不包含引号，用冒号隔开的若干整数，每个代表一级1/2下抽样的整数位移量|
|firshift|抗混叠滤波器的整数位移量|可选，不设或为空字符串则禁用抗混叠滤波器|

注意：实际采样率等于 `320 MSps/(1<<ns)`，其中 ns 为 `shifts` 的个数（每级 1/2 下抽样使采样率减半）。
